...
public class ReceiveOrderBehaviour extends Behaviour{
	...
	public ReceiveOrderBehaviour(ResourceAgent myAgent, String conversationID, long reply_by_time, long reply_by_date_long, int proposalNumber) {
		super(myAgent);
		...
		//System.out.println("__________________________"+myAgent.getLocalName()+" offerID at ReceiveOrderBehav creation"+offerNumber);
	}
	
	public void onStart(){
		/*
		try {
			Thread.sleep((long)0.1*reply_by_time);
...
//checks if reply by time has expired
		...
		case 1:
		 ...
		 ACLMessage order = myAgent.receive(mt_total);
			if (order != null) {
				...
				if(myAgent.getNeeded_shared_resources().size()>0) {
					this.getDataStore().put(3, true);
					//now wait in step X until inf. sched of shared resources is available
					step = 4;
				}else {
					step = 3;
				}
			...
			break;
		case 2:
			//System.out.println(System.currentTimeMillis()+" "+myAgent.SimpleDateFormat.format(new Date())+" "+myAgent.getLocalName()+logLinePrefix+"no order received for offer "+proposal_id+" with conversation id "+conversationID);
			this.getDataStore().put(3, false);
			step = 5;
		...
		case 3:
			...
			*/
			...
			//create Gantt chart and add to database for transport resources --> production resources do that when the departure time is known
		...
		case 4:	//wait until both inf.alloc have been received
			/*if(System.currentTimeMillis() >= order_received_at_date+0.5*myAgent.reply_by_time_shared_resources) {*/
			
				Boolean allValuesTrue = false;
				String string = "";
				int counter = 0;
				for(Boolean a : 	((Boolean[]) this.getDataStore().get(2))) {
					string = string + " "+counter+" "+a+" ";
					if(a == true &&  counter == 0) {
						allValuesTrue = true;
					}else if(a == true &&  allValuesTrue == true){
