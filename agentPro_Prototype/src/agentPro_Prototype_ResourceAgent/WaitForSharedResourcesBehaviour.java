...
public class WaitForSharedResourcesBehaviour extends Behaviour{
	...
	public void action() {
					...
					if(shared_resource_asked[j] == false) {	
						...
						for(int k = 0;k<myAgent.getNeeded_shared_resources().size();k++) {
							...
							//}else if(j == myAgent.getNeeded_shared_resources().size()-1) {
									...
									Boolean intersection_ok = checkProposalIntersection();
								if(intersection_ok) {
									...
									if(Long.parseLong(best_available_timeslot.getEndDate()) <= Long.parseLong(timeslot_for_schedule.getEndDate())+requested_operation.getBuffer_after_operation() && Long.parseLong(best_available_timeslot.getStartDate()) >= Long.parseLong(timeslot_for_schedule.getStartDate())+requested_operation.getBuffer_before_operation()) {
										System.out.println("DEBUG_________TIMESLOT "+best_available_timeslot.getStartDate()+","+best_available_timeslot.getEndDate()+" is apropriate because end is <= "+(Long.parseLong(timeslot_for_schedule.getEndDate())+requested_operation.getBuffer_before_operation()));
		...
		case 1:
			...
			Boolean bool = 	myAgent.getReceiveCFPBehav().bookIntoSchedule(for_schedule, time_increment_or_decrement_to_be_added_for_setup_of_next_task);
			if(!bool) {
	...
	private Timeslot determineBestTimeslotFromSharedResources() {
		...
		*/
		...
		Interval transport_resource = new Interval(Long.parseLong(timeslot_for_schedule.getStartDate()) - (long) requested_operation.getBuffer_before_operation(), Long.parseLong(timeslot_for_schedule.getEndDate()) + (long) requested_operation.getBuffer_after_operation());
