...
public class Geometry {
    ...
    public static Set<LineSegment[]> getAllIntersectingLines(LineSegment[] lines) {
        ...
        for (int i = 0; i < eventPointSchedule.size(); i++) {
            int j = i + 1;
            //System.out.println("DEBUG_____FOR WHILE_"+eventPointSchedule.size()+" j = "+j);
                            ...
                            - eventPointSchedule.get(j).sortingKey) < EPSILON) 
            {
            	...
            	if(j<eventPointSchedule.size()) {					//FELIX
                    LineSegment[] tmp = new LineSegment[2];
    ...
    public static Set<LineSegment[]> getAllIntersectingLinesByBruteForce(
        ...
        for (int i = 0; i < lines.length; i++) {
            for (int j = i + 1; j < lines.length; j++) {
            	if(lines[i].second.x == lines[j].first.x && lines[i].second.y == lines[j].first.y) {	//FElIX
            		
            	}else if(lines[i].first.x == lines[j].second.x && lines[i].first.y == lines[j].second.y){
